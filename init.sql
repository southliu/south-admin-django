...
-- 插入顶级菜单项
INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id) VALUES
('仪表盘', 'Dashboard', 'la:tachometer-alt', '/dashboard', '/dashboard', 0, 1, NOW(), NOW(), NULL),
('组件', 'Components', 'fluent:box-20-regular', '/demo', NULL, 1, 1, NOW(), NOW(), NULL),
('系统管理', 'System Management', 'ion:settings-outline', '/system', NULL, 2, 1, NOW(), NOW(), NULL),
('内容管理', 'Content Management', 'majesticons:article-search-line', '/content', NULL, 3, 1, NOW(), NOW(), NULL),
('外部链接', 'External Link', 'material-symbols:link', 'https://ant-design.antgroup.com', '/dashboard', 4, 1, NOW(), NOW(), NULL);

-- 插入组件子菜单（使用派生表解决子查询问题）
INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id)
SELECT '剪切板', 'Copy', NULL, '/demo/copy', '/demo/copy', 0, 1, NOW(), NOW(), parent_menu.id
FROM (SELECT id FROM menu WHERE router = '/demo') AS parent_menu;

INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id)
SELECT '水印', 'Watermark', NULL, '/demo/watermark', '/demo/watermark', 1, 1, NOW(), NOW(), parent_menu.id
FROM (SELECT id FROM menu WHERE router = '/demo') AS parent_menu;

INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id)
SELECT '虚拟滚动', 'Virtual Scroll', NULL, '/demo/virtualScroll', '/demo/virtualScroll', 2, 1, NOW(), NOW(), parent_menu.id
FROM (SELECT id FROM menu WHERE router = '/demo') AS parent_menu;

INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id)
SELECT '富文本', 'Editor', NULL, '/demo/editor', '/demo/editor', 3, 1, NOW(), NOW(), parent_menu.id
FROM (SELECT id FROM menu WHERE router = '/demo') AS parent_menu;

INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id)
SELECT '动态路由参数', 'Dynamic', NULL, '/demo/123/dynamic', '/demo/dynamic', 4, 1, NOW(), NOW(), parent_menu.id
FROM (SELECT id FROM menu WHERE router = '/demo') AS parent_menu;

INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id)
SELECT '层级1', 'Level1', NULL, '/demo/level1', NULL, 5, 1, NOW(), NOW(), parent_menu.id
FROM (SELECT id FROM menu WHERE router = '/demo') AS parent_menu;

-- 插入层级子菜单
INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id)
SELECT '层级2', 'Level2', NULL, '/demo/level1/level2', NULL, 0, 1, NOW(), NOW(), parent_menu.id
FROM (SELECT id FROM menu WHERE router = '/demo/level1') AS parent_menu;

INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id)
SELECT '层级3', 'Level3', NULL, '/demo/level1/level2/level3', '/demo/watermark', 0, 1, NOW(), NOW(), parent_menu.id
FROM (SELECT id FROM menu WHERE router = '/demo/level1/level2') AS parent_menu;

-- 插入系统管理子菜单
INSERT INTO menu (label, labelEn, icon, router, rule, `order`, is_visible, created_at, updated_at, parent_id)
SELECT '用户管理', 'User Management', NULL, '/system/user', '/authority/user', 0, 1, NOW(), NOW(), parent_menu.id
